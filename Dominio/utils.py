...
import Dominio.busquedas as busquedas
...
def mezclar_aleatorio(num_movimientos, cubo):
    '''
    Realiza un movimiento aleatorio del cubo
    '''
    tipoMov = ['B', 'b', 'L', 'l', 'D', 'd']
...
def moverCubo(cubo, movimiento, fila):
    ...
    if movimiento == 'B':
        cubo.desplazamientoB(fila)
    if movimiento == 'b':
        cubo.desplazamientob(fila)
    if movimiento == 'L':
        cubo.desplazamientoL(fila)
    if movimiento == 'l':
        cubo.desplazamientol(fila)
    if movimiento == 'D':
        cubo.desplazamientoD(fila)
    if movimiento == 'd':
        cubo.desplazamientod(fila)
    cubo.updateEstado()
    if generar_imagenes:
...
def resolverCubo(problema):
    ...
    listaSolucion = []
    optEstrategias = dict(
        {1: "profundidad", 2: "anchura", 3: "costo", 4: "Aestrella", 5: "voraz"})
    opt = int(input(
        "¿Cómo quieres resolver el cubo:\n 1.Profundidad\n 2.Anchura\n 3.Costo\n 4.A*\n 5.Voraz\n0.Cancelar\n"))
...
def resolverAll(problema):
    ...
    for estrategia in optEstrategias:
        ...
        listaSolucion = busquedas.busquedaAcotada(
            problema, estrategia, profMax, opti=optimizacion)
        print(f'Tiempo transcurrido: {time.time() - t_inicial}')
